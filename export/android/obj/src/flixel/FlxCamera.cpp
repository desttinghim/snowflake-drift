...
Float Zoom = __o_Zoom.Default(0);
{
	...
	HX_STACK_LINE(447)
	this->canvas = ::openfl::_v2::display::Sprite_obj::__new();
	HX_STACK_LINE(448)
	this->canvas->set_x((-(this->width) * 0.5));
	HX_STACK_LINE(449)
	this->canvas->set_y((-(this->height) * 0.5));
	HX_STACK_LINE(456)
	this->flashSprite = ::openfl::_v2::display::Sprite_obj::__new();
	HX_STACK_LINE(457)
	this->set_zoom(Zoom);
	HX_STACK_LINE(459)
	...
	HX_STACK_LINE(469)
	this->_flashRect = ::openfl::_v2::geom::Rectangle_obj::__new((int)0,(int)0,this->width,this->height);
	HX_STACK_LINE(470)
	...
	HX_STACK_LINE(472)
	{
		...
		point->_inPool = false;
		HX_STACK_LINE(472)
		this->_fxShakeOffset = point;
	}
	HX_STACK_LINE(478)
	this->canvas->set_scrollRect(::openfl::_v2::geom::Rectangle_obj::__new((int)0,(int)0,this->width,this->height));
	HX_STACK_LINE(481)
	this->debugLayer = ::openfl::_v2::display::Sprite_obj::__new();
	HX_STACK_LINE(482)
...
Void FlxCamera_obj::render( ){
{
		...
		while(((currItem != null()))){
			...
			if (((position > (int)0))){
				...
				HX_STACK_LINE(401)
				currItem->graphics->get_tilesheet()->tileSheet->drawTiles(this->canvas->get_graphics(),data,(bool(this->antialiasing) || bool(currItem->antialiasing)),tempFlags,position);
				HX_STACK_LINE(402)
...
Void FlxCamera_obj::destroy( ){
{
		...
		HX_STACK_LINE(526)
		this->scroll = ::flixel::util::FlxDestroyUtil_obj::put(this->scroll);
		HX_STACK_LINE(527)
		this->deadzone = ::flixel::util::FlxDestroyUtil_obj::put(this->deadzone);
		HX_STACK_LINE(528)
		this->bounds = ::flixel::util::FlxDestroyUtil_obj::put(this->bounds);
		HX_STACK_LINE(530)
		this->target = null();
		HX_STACK_LINE(531)
...
int FlxCamera_obj::set_width( int Value){
	...
	if (((Value > (int)0))){
		...
		if (((this->canvas != null()))){
			HX_STACK_LINE(1100)
			::openfl::_v2::geom::Rectangle rect = this->canvas->get_scrollRect();		HX_STACK_VAR(rect,"rect");
			HX_STACK_LINE(1101)
			rect->width = Value;
			HX_STACK_LINE(1102)
...
int FlxCamera_obj::set_color( int Color){
	...
	HX_STACK_LINE(1190)
	::openfl::_v2::geom::ColorTransform colorTransform = this->canvas->get_transform()->get_colorTransform();		HX_STACK_VAR(colorTransform,"colorTransform");
	HX_STACK_LINE(1191)
