		...
		HX_STACK_LINE(556)
		{
			...
			while(((_g < l))){
				...
				if ((this->_sprite->framesData->framesHash->exists(name))){
					...
					HX_STACK_LINE(562)
					AddTo->push(this->_sprite->framesData->frames->indexOf(frameToAdd,null()));
				}
			}
		}
	}
...
Void FlxAnimationController_obj::byPrefixHelper( Array< int > AddTo,Array< ::Dynamic > AnimFrames,::String Prefix){
{
		...
		HX_STACK_LINE(613)
		{
			...
			while(((_g < l))){
				...
				int i = (_g)++;		HX_STACK_VAR(i,"i");
				HX_STACK_LINE(615)
				AddTo->push(this->_sprite->framesData->frames->indexOf(AnimFrames->__get(i).StaticCast< ::flixel::system::layer::frames::FlxFrame >(),null()));
			}
		}
	}
return null();
...
::String FlxAnimationController_obj::set_frameName( ::String Value){
	...
	if (((  (((this->_sprite->framesData != null()))) ? bool(this->_sprite->framesData->framesHash->exists(Value)) : bool(false) ))){
		...
		if (((frame != null()))){
			HX_STACK_LINE(668)
			this->set_frameIndex(this->_sprite->framesData->frames->indexOf(frame,null()));
		}
	}
	HX_STACK_LINE(672)
	return Value;
