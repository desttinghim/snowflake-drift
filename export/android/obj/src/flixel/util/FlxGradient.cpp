	...
	HX_STACK_ARG(interpolate,"interpolate")
{
		...
		HX_STACK_LINE(83)
		{
			...
			while(((_g1 < _g))){
				...
				HX_STACK_LINE(85)
				result->push(data->getPixel32((int)0,y));
			}
		}
		HX_STACK_LINE(88)
		return result;
	...
	HX_STACK_ARG(interpolate,"interpolate")
{
		...
		struct _Function_1_1{
			inline static bool Block( ::String &key){
				HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","flixel/util/FlxGradient.hx",137,0x5a5037d6)
				{
					...
					HX_STACK_LINE(137)
					return (  ((_this->_cache->exists(key))) ? bool((_this->_cache->get(key) != null())) : bool(false) );
				}
				return null();
			}
		};
		...
		HX_STACK_LINE(145)
		::openfl::_v2::display::Shape s = ::openfl::_v2::display::Shape_obj::__new();		HX_STACK_VAR(s,"s");
		HX_STACK_LINE(147)
		if ((interpolate)){
			HX_STACK_LINE(150)
			s->get_graphics()->beginGradientFill(::openfl::display::GradientType_obj::LINEAR,colors,gradient->__Field(HX_CSTRING("alpha"),true),gradient->__Field(HX_CSTRING("ratio"),true),gradient->__Field(HX_CSTRING("matrix"),true),::openfl::_v2::display::SpreadMethod_obj::PAD,::openfl::display::InterpolationMethod_obj::RGB,(int)0);
		}
		else{
			HX_STACK_LINE(156)
			s->get_graphics()->beginGradientFill(::openfl::display::GradientType_obj::LINEAR,colors,gradient->__Field(HX_CSTRING("alpha"),true),gradient->__Field(HX_CSTRING("ratio"),true),gradient->__Field(HX_CSTRING("matrix"),true),::openfl::_v2::display::SpreadMethod_obj::PAD,::openfl::display::InterpolationMethod_obj::LINEAR_RGB,(int)0);
		}
		HX_STACK_LINE(160)
		if (((chunkSize == (int)1))){
			HX_STACK_LINE(162)
		...
		if (((chunkSize == (int)1))){
			...
			HX_STACK_LINE(177)
			::openfl::_v2::display::Bitmap tempBitmap = ::openfl::_v2::display::Bitmap_obj::__new(::openfl::_v2::display::BitmapData_obj::__new(width,::Std_obj::_int((Float(height) / Float(chunkSize))),true,(int)0,null()),null(),null());		HX_STACK_VAR(tempBitmap,"tempBitmap");
			HX_STACK_LINE(178)
			...
			HX_STACK_LINE(182)
			Float _g = tempBitmap->get_scaleX();		HX_STACK_VAR(_g,"_g");
			HX_STACK_LINE(182)
			sM->scale(_g,tempBitmap->get_scaleY());
			HX_STACK_LINE(184)
	...
	HX_STACK_ARG(interpolate,"interpolate")
{
		...
		if (((width > dest->get_width()))){
			HX_STACK_LINE(213)
			width = ::Std_obj::_int(dest->get_width());
		}
		HX_STACK_LINE(216)
		if (((height > dest->get_height()))){
			HX_STACK_LINE(218)
			height = ::Std_obj::_int(dest->get_height());
		}
		HX_STACK_LINE(221)
	...
	HX_STACK_ARG(interpolate,"interpolate")
{
		...
		if (((width > dest->get_width()))){
			HX_STACK_LINE(246)
			width = dest->get_width();
		}
		HX_STACK_LINE(249)
		if (((height > dest->get_height()))){
			HX_STACK_LINE(251)
			height = dest->get_height();
		}
		HX_STACK_LINE(254)
		...
		HX_STACK_LINE(255)
		::openfl::_v2::geom::Rectangle _g1 = ::openfl::_v2::geom::Rectangle_obj::__new((int)0,(int)0,_g,source->get_height());		HX_STACK_VAR(_g1,"_g1");
		HX_STACK_LINE(255)
		dest->copyPixels(source,_g1,::openfl::_v2::geom::Point_obj::__new(destX,destY),null(),null(),true);
		HX_STACK_LINE(256)
		source->dispose();
		HX_STACK_LINE(257)
		return dest;
