...
int height = __o_height.Default(0);
{
	...
	if (((width <= (int)0))){
		HX_STACK_LINE(16)
		width = data->bitmap->get_width();
	}
	HX_STACK_LINE(17)
	if (((height <= (int)0))){
		HX_STACK_LINE(17)
		height = data->bitmap->get_height();
	}
	HX_STACK_LINE(19)
	this->region = ::flixel::system::layer::Region_obj::__new(startX,startY,tileWidth,tileHeight,spacingX,spacingY,width,height);
}
;
	return null();
}
